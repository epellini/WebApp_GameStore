$(document).ready(function () {
    $('#searchInput').keyup(function () {
        var query = $(this).val();
        url = "games/search";
        $.ajax({
            type: "Get",
            url: url,
            data: { 'query': query },
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            success: function (result) {
                console.log(result);
                if (result.sort === '') {
                    var sortElements = document.getElementsByClassName("sort-link-snow-active");
                    for (var i = 0; i < sortElements.length; i++) {
                        sortElements[i].classList.remove("sort-link-snow-active");
                    }
                }
                if (result.sort === 'New') {
                    var sortElements = document.getElementsByClassName("sort-link-snow");
                    sortElements[0].classList.add("sort-link-snow-active");
                }
                
                $('#gamesList').html('');
                for (var i = 0; i < result.games.length; i++) {
                    let picture = `<img src="/images/no-image.png" class="card-img-top p-0" alt="${result.games[i].Name} cover image" style="object-fit: cover">`;
                    if (result.pictures[i][0] != null) {
                        picture = `<img src="/images/${result.pictures[i][0].pictureId}" class="card-img-top p-0" alt="${result.pictures[i][0].altText}" style="object-fit: cover">`;
                    }
                    let price = `<p class="m-0 p-1 fw-bold">Free to play</p>`;
                    if (result.games[i].retailPrice != 0) {
                        price = `<p class="m-0 p-1 fw-bold">CA$${result.games[i].retailPrice}</p>`; 
                    }

                    let platforms = '';
                    for (var j = 0; j < result.platforms[i].length; j++) {
                        if (result.platforms[i][j].platformName == 'Windows') {
                            platforms += `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 640 640" fill="#4c566a"><path d="M.2 298.669L0 90.615l256.007-34.76v242.814H.201zM298.658 49.654L639.905-.012v298.681H298.657V49.654zM640 341.331l-.071 298.681L298.669 592V341.332h341.33zM255.983 586.543L.189 551.463v-210.18h255.794v245.26z" /></svg>\n`;
                        }
                        if (result.platforms[i][j].platformName == 'Mac') {
                            platforms += `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#4c566a" height="16" width="16" version="1.1" id="Capa_1" viewBox="0 0 22.773 22.773" xml:space="preserve"><path d="M15.769,0c0.053,0,0.106,0,0.162,0c0.13,1.606-0.483,2.806-1.228,3.675c-0.731,0.863-1.732,1.7-3.351,1.573    c-0.108-1.583,0.506-2.694,1.25-3.561C13.292,0.879,14.557,0.16,15.769,0z" />
                                                <path d="M20.67,16.716c0,0.016,0,0.03,0,0.045c-0.455,1.378-1.104,2.559-1.896,3.655c-0.723,0.995-1.609,2.334-3.191,2.334    c-1.367,0-2.275-0.879-3.676-0.903c-1.482-0.024-2.297,0.735-3.652,0.926c-0.155,0-0.31,0-0.462,0    c-0.995-0.144-1.798-0.932-2.383-1.642c-1.725-2.098-3.058-4.808-3.306-8.276c0-0.34,0-0.679,0-1.019    c0.105-2.482,1.311-4.5,2.914-5.478c0.846-0.52,2.009-0.963,3.304-0.765c0.555,0.086,1.122,0.276,1.619,0.464    c0.471,0.181,1.06,0.502,1.618,0.485c0.378-0.011,0.754-0.208,1.135-0.347c1.116-0.403,2.21-0.865,3.652-0.648    c1.733,0.262,2.963,1.032,3.723,2.22c-1.466,0.933-2.625,2.339-2.427,4.74C17.818,14.688,19.086,15.964,20.67,16.716z" /></svg>\n`;
                        }
                        if (result.platforms[i][j].platformName == 'Linux') {
                            platforms += `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#4c566a" height="16" width="16" version="1.1" id="Layer_1" viewBox="0 0 304.998 304.998" xml:space="preserve"><g id="XMLID_91_"><path id="XMLID_92_" d="M274.659,244.888c-8.944-3.663-12.77-8.524-12.4-15.777c0.381-8.466-4.422-14.667-6.703-17.117   c1.378-5.264,5.405-23.474,0.004-39.291c-5.804-16.93-23.524-42.787-41.808-68.204c-7.485-10.438-7.839-21.784-8.248-34.922   c-0.392-12.531-0.834-26.735-7.822-42.525C190.084,9.859,174.838,0,155.851,0c-11.295,0-22.889,3.53-31.811,9.684   c-18.27,12.609-15.855,40.1-14.257,58.291c0.219,2.491,0.425,4.844,0.545,6.853c1.064,17.816,0.096,27.206-1.17,30.06   c-0.819,1.865-4.851,7.173-9.118,12.793c-4.413,5.812-9.416,12.4-13.517,18.539c-4.893,7.387-8.843,18.678-12.663,29.597   c-2.795,7.99-5.435,15.537-8.005,20.047c-4.871,8.676-3.659,16.766-2.647,20.505c-1.844,1.281-4.508,3.803-6.757,8.557   c-2.718,5.8-8.233,8.917-19.701,11.122c-5.27,1.078-8.904,3.294-10.804,6.586c-2.765,4.791-1.259,10.811,0.115,14.925   c2.03,6.048,0.765,9.876-1.535,16.826c-0.53,1.604-1.131,3.42-1.74,5.423c-0.959,3.161-0.613,6.035,1.026,8.542   c4.331,6.621,16.969,8.956,29.979,10.492c7.768,0.922,16.27,4.029,24.493,7.035c8.057,2.944,16.388,5.989,23.961,6.913   c1.151,0.145,2.291,0.218,3.39,0.218c11.434,0,16.6-7.587,18.238-10.704c4.107-0.838,18.272-3.522,32.871-3.882   c14.576-0.416,28.679,2.462,32.674,3.357c1.256,2.404,4.567,7.895,9.845,10.724c2.901,1.586,6.938,2.495,11.073,2.495   c0.001,0,0,0,0.001,0c4.416,0,12.817-1.044,19.466-8.039c6.632-7.028,23.202-16,35.302-22.551c2.7-1.462,5.226-2.83,7.441-4.065   c6.797-3.768,10.506-9.152,10.175-14.771C282.445,250.905,279.356,246.811,274.659,244.888z M124.189,243.535   c-0.846-5.96-8.513-11.871-17.392-18.715c-7.26-5.597-15.489-11.94-17.756-17.312c-4.685-11.082-0.992-30.568,5.447-40.602   c3.182-5.024,5.781-12.643,8.295-20.011c2.714-7.956,5.521-16.182,8.66-19.783c4.971-5.622,9.565-16.561,10.379-25.182   c4.655,4.444,11.876,10.083,18.547,10.083c1.027,0,2.024-0.134,2.977-0.403c4.564-1.318,11.277-5.197,17.769-8.947   c5.597-3.234,12.499-7.222,15.096-7.585c4.453,6.394,30.328,63.655,32.972,82.044c2.092,14.55-0.118,26.578-1.229,31.289   c-0.894-0.122-1.96-0.221-3.08-0.221c-7.207,0-9.115,3.934-9.612,6.283c-1.278,6.103-1.413,25.618-1.427,30.003   c-2.606,3.311-15.785,18.903-34.706,21.706c-7.707,1.12-14.904,1.688-21.39,1.688c-5.544,0-9.082-0.428-10.551-0.651l-9.508-10.879   C121.429,254.489,125.177,250.583,124.189,243.535z M136.254,64.149c-0.297,0.128-0.589,0.265-0.876,0.411   c-0.029-0.644-0.096-1.297-0.199-1.952c-1.038-5.975-5-10.312-9.419-10.312c-0.327,0-0.656,0.025-1.017,0.08   c-2.629,0.438-4.691,2.413-5.821,5.213c0.991-6.144,4.472-10.693,8.602-10.693c4.85,0,8.947,6.536,8.947,14.272   C136.471,62.143,136.4,63.113,136.254,64.149z M173.94,68.756c0.444-1.414,0.684-2.944,0.684-4.532   c0-7.014-4.45-12.509-10.131-12.509c-5.552,0-10.069,5.611-10.069,12.509c0,0.47,0.023,0.941,0.067,1.411   c-0.294-0.113-0.581-0.223-0.861-0.329c-0.639-1.935-0.962-3.954-0.962-6.015c0-8.387,5.36-15.211,11.95-15.211   c6.589,0,11.95,6.824,11.95,15.211C176.568,62.78,175.605,66.11,173.94,68.756z M169.081,85.08   c-0.095,0.424-0.297,0.612-2.531,1.774c-1.128,0.587-2.532,1.318-4.289,2.388l-1.174,0.711c-4.718,2.86-15.765,9.559-18.764,9.952   c-2.037,0.274-3.297-0.516-6.13-2.441c-0.639-0.435-1.319-0.897-2.044-1.362c-5.107-3.351-8.392-7.042-8.763-8.485   c1.665-1.287,5.792-4.508,7.905-6.415c4.289-3.988,8.605-6.668,10.741-6.668c0.113,0,0.215,0.008,0.321,0.028   c2.51,0.443,8.701,2.914,13.223,4.718c2.09,0.834,3.895,1.554,5.165,2.01C166.742,82.664,168.828,84.422,169.081,85.08z    M205.028,271.45c2.257-10.181,4.857-24.031,4.436-32.196c-0.097-1.855-0.261-3.874-0.42-5.826   c-0.297-3.65-0.738-9.075-0.283-10.684c0.09-0.042,0.19-0.078,0.301-0.109c0.019,4.668,1.033,13.979,8.479,17.226   c2.219,0.968,4.755,1.458,7.537,1.458c7.459,0,15.735-3.659,19.125-7.049c1.996-1.996,3.675-4.438,4.851-6.372   c0.257,0.753,0.415,1.737,0.332,3.005c-0.443,6.885,2.903,16.019,9.271,19.385l0.927,0.487c2.268,1.19,8.292,4.353,8.389,5.853   c-0.001,0.001-0.051,0.177-0.387,0.489c-1.509,1.379-6.82,4.091-11.956,6.714c-9.111,4.652-19.438,9.925-24.076,14.803   c-6.53,6.872-13.916,11.488-18.376,11.488c-0.537,0-1.026-0.068-1.461-0.206C206.873,288.406,202.886,281.417,205.028,271.45z    M39.917,245.477c-0.494-2.312-0.884-4.137-0.465-5.905c0.304-1.31,6.771-2.714,9.533-3.313c3.883-0.843,7.899-1.714,10.525-3.308   c3.551-2.151,5.474-6.118,7.17-9.618c1.228-2.531,2.496-5.148,4.005-6.007c0.085-0.05,0.215-0.108,0.463-0.108   c2.827,0,8.759,5.943,12.177,11.262c0.867,1.341,2.473,4.028,4.331,7.139c5.557,9.298,13.166,22.033,17.14,26.301   c3.581,3.837,9.378,11.214,7.952,17.541c-1.044,4.909-6.602,8.901-7.913,9.784c-0.476,0.108-1.065,0.163-1.758,0.163   c-7.606,0-22.662-6.328-30.751-9.728l-1.197-0.503c-4.517-1.894-11.891-3.087-19.022-4.241c-5.674-0.919-13.444-2.176-14.732-3.312   c-1.044-1.171,0.167-4.978,1.235-8.337c0.769-2.414,1.563-4.91,1.998-7.523C41.225,251.596,40.499,248.203,39.917,245.477z" /></g></svg>\n`;
                        }
                        if (result.platforms[i][j].platformName == 'MS-DOS') {
                            platforms += `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 607 772.79" height="16" width="16" fill="#4c566a"><g transform="translate(-38,-52)"><path d="m38 59v366h72v-212l50 212h70l50-212v212h72v-366h-105l-52 217-52-217z" /><path d="m500 52c-96 0-132 63.721-132 118 0 126 200 85.221 200 148 0 23.314-6.8417 44-64 44s-64-51-64-51h-75c0 54 50 118 140 118 97.941 0 140-63.903 140-118 0-119.85-203-93-203-147 0-32.918 30-44 60-44 50.984 0 60 50 60 50h70c0-54.097-48-118-132-118z" /><path d="m38 449v367.7h152.65s144.6-6.4495 144.6-183.85c0-43.684-8.7917-76.961-22.01-102.37-14.318 4.9682-23.807 13.301-30.881 24.174-11.836 18.193-16.311 45.901-16.311 78.043 0 111.4-71.268 111.7-71.268 111.7h-83.447v-223.1h66.104c9.1888-18.78 20.95-34.543 34.949-47.344 6.2038-5.6727 12.819-10.707 19.746-15.221-23.376-8.8872-41.482-9.7363-41.482-9.7363z" /><path d="m349.5 441.3c-100.24 0-181.5 48.868-181.5 191.4 0 39.924 6.3828 72.491 17.766 98.701h8.9609s13.132 0.11436 27.656-11.299c14.524-11.413 30.666-34.476 30.666-87.402 0-65.305 16.395-118.43 92.293-120.31 5.4475-15.611 14.128-30.455 26.783-43.186 8.6221-8.6737 19.027-16.221 31.205-22.387-17.009-3.7669-35.087-5.5176-53.83-5.5176z" /><path d="m543.14 608.6c0.55081 7.7612 0.86328 15.769 0.86328 24.096 0 17.655-1.2958 34.032-3.7012 49.254 17.824 6.964 29.477 15.177 29.477 27.281 0 24.763-9.2871 48.494-77.385 48.494-74.289 0-76.352-57.779-76.352-57.779h-74.289c0 61.907 52.622 124.85 151.67 124.85 105.24 0 148.58-51.153 148.58-119.69 0-55.808-47.641-80.598-98.863-96.504z" /><path d="m488.26 442c-102.15 0-137.23 58.813-137.23 114.53 0 49.989 37.526 75.71 81.479 92.461 0.28599-5.2843 0.43555-10.713 0.43555-16.293 0-27.254-3.1559-51.381-11.387-69.076-1.5796-2.7915-2.4297-5.8234-2.4297-9.1562 0-33.063 30.953-45.398 71.193-45.398 65.002 0 67.066 51.59 67.066 51.59h73.256c0-68.098-55.717-118.66-142.39-118.66z" /><path d="m319.6 749.98c-14.05 23.558-31.737 40.485-49.506 52.178-2.9783 1.9598-5.9528 3.736-8.918 5.4277 26.145 11.471 56.252 16.508 88.322 16.508 19.554 0 38.37-1.934 56.018-6.0469-30.314-15.483-52.246-38.774-64.713-65.199-7.6898-0.42133-14.74-1.3922-21.203-2.8672z" /><path d="m490.33 522.07c-18.792 0-34.647 3.1663-44.291 8.6172-9.6442 5.451-13.902 11.306-13.902 23.781 0 0.50284 0.0416 0.95316 0.12695 1.3906 10.411 20.85 13.689 47.629 13.689 76.842 0 29.784-3.4262 57.022-14.326 78.041 1.1982 3.2922 2.9081 6.9964 5.4883 10.799 7.6597 11.288 21.503 23.189 55.277 23.189 5.2088 0 9.8573-0.1998 14.26-0.48242 15.457-28.241 24.35-64.891 24.35-111.55 0-45.292-8.3718-81.164-23-109.06-5.0447-0.98932-10.86-1.5703-17.672-1.5703z" /></g></svg>\n`;
                        }
                    }
                    let detailsUrl = window.location.origin + `/games/${result.games[i].gameId}`
                    $('#gamesList').append(`
                    <div class="col-xl-3 col-lg-4 col-12 col-sm-6 p-0">
                        <div class="card shadow bg-night text-snow-bright m-2">
                            <div class="row m-0 ratio ratio-16x9">
                                ${picture}
                            </div>
                            <div class="row text-start p-2 m-0 bg-shade">
                                <h6 class="p-1 m-0 card-title text-uppercase fw-bold col-10 overflow-hidden" style="height: 3rem">
                                    ${result.games[i].name}
                                </h6>
                                <div class="col-2 p-0">
                                    <div class="row justify-content-end p-0 m-0">
                                        <span role="button" class="col-auto p-0 pe-1" id="" onclick="">
                                            <svg class="svg-icon svg-icon-snow" id="" fill="" width="18" height="18" viewBox="0 0 16 16"> <path d="M7.84 4.1a.178.178 0 0 1 .32 0l.634 1.285a.178.178 0 0 0 .134.098l1.42.206c.145.021.204.2.098.303L9.42 6.993a.178.178 0 0 0-.051.158l.242 1.414a.178.178 0 0 1-.258.187l-1.27-.668a.178.178 0 0 0-.165 0l-1.27.668a.178.178 0 0 1-.257-.187l.242-1.414a.178.178 0 0 0-.05-.158l-1.03-1.001a.178.178 0 0 1 .098-.303l1.42-.206a.178.178 0 0 0 .134-.098L7.84 4.1z"/><path d="M2 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v13.5a.5.5 0 0 1-.777.416L8 13.101l-5.223 2.815A.5.5 0 0 1 2 15.5V2zm2-1a1 1 0 0 0-1 1v12.566l4.723-2.482a.5.5 0 0 1 .554 0L13 14.566V2a1 1 0 0 0-1-1H4z"/></svg>
                                        </span>
                                    </div>

                                </div>
                            </div>
                            <div class="card-body text-start m-0 p-0 text-uppercase">
                                <div class="row justify-content-between p-2 m-0">
                                    <div class="col-auto p-1">
                                        ${platforms}
                                    </div>
                                    <div class="col-auto p-0">
                                        ${price}
                                    </div>
                                </div>
                                <div class="row justify-content-between p-2 m-0">
                                    <div class="col-6 px-1">
                                        <a href="${detailsUrl}" class="col-12 btn btn-frost-outline">Details</a>
                                    </div>
                                    <div class="col-6 px-1">
                                        <a href="#" class="col-12 btn btn-frost-primary">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="20" fill="none" stroke="#2E3440" viewBox="0 0 24 28">
                                                <path d="M6.29977 5H21L19 12H7.37671M20 16H8L6 3H3M9 20C9 20.5523 8.55228 21 8 21C7.44772 21 7 20.5523 7 20C7 19.4477 7.44772 19 8 19C8.55228 19 9 19.4477 9 20ZM20 20C20 20.5523 19.5523 21 19 21C18.4477 21 18 20.5523 18 20C18 19.4477 18.4477 19 19 19C19.5523 19 20 19.4477 20 20Z" stroke="#2E3440" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                            </svg>
                                            Add
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    `);
                }
            }
        });
    });
    $(".star").on("click", function () {

        $(".star").find('svg').html(`<path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z" />`);
        $(this).find('svg').html(`<path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>`);
        $(this).prevAll().find('svg').html(`<path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>`);

        let rate = $(this).attr('id').split('-')[2].trim();
        let id = $(this).attr('id').split('-')[0].trim();

        console.log(rate);

        url = `/games/${id}/rate/${rate}`;
        $.ajax({
            type: "Post",
            url: url,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: true,
            success: function (result) {
                console.log(result);
                if (result.added) {
                    console.log(result.average);
                    $("#averageRating").text(result.average);
                }
            }
        });

    });
});

function addGameToWishlist(id) {
    //console.log("Game: " + id + " is being wishlisted.");

    url = `/games/${id}/add-to-wishlist`;
    $.ajax({
        type: "Post",
        url: url,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
        success: function (result) {
            //console.log(result);
            if (result.added) {
                if ($("#WishlistBtn").hasClass('btn-frost-outline')) {
                    $("#WishlistBtn").text('Remove from Wishlist');
                    $("#WishlistBtn").removeClass('btn-frost-outline');
                    $("#WishlistBtn").addClass('btn-aurora-red-outline');
                }
                else if ($("#WishlistBtn").hasClass('btn-aurora-red-outline')) {
                    $("#WishlistBtn").text('Add to Wishlist');
                    $("#WishlistBtn").removeClass('btn-aurora-red-outline');
                    $("#WishlistBtn").addClass('btn-frost-outline');
                }
                $(`.WishlistIcon-${id}`).toggleClass('d-none');
            }
        }
    });
}

function registerForEvent(id) {
    url = `/events/${id}/register`;
    $.ajax({
        type: "Post",
        url: url,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
        success: function (result) {
            //console.log(result);
            if (result.added) {
                if ($("#RegisterBtn").hasClass('btn-frost-outline')) {
                    $("#RegisterBtn").text('Deregister');
                    $("#RegisterBtn").removeClass('btn-frost-outline');
                    $("#RegisterBtn").addClass('btn-aurora-red-outline');
                }
                else if ($("#RegisterBtn").hasClass('btn-aurora-red-outline')) {
                    $("#RegisterBtn").text('Register');
                    $("#RegisterBtn").removeClass('btn-aurora-red-outline');
                    $("#RegisterBtn").addClass('btn-frost-outline');
                }
                $("#regCount").text('');
                $("#regCount").append(`Members Registered: <b id="regCount">${result.count}</b></p>`);
            }
        }
    });
}

function openReview() {
    $("#review-form").toggleClass('d-none');
}

